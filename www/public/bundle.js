(function () {
    ...
    ***************************************************************************** */
    ...
    var createEmitter = function () {
        ...
        function on(event) {
            ...
            event.map(function (e) {
                ...
                (_a = _listeners[e]) === null || _a === void 0 ? void 0 : _a.push.apply(_a, __spreadArray([], __read(handlers), false)); // TODO: remove ? later in the future versions of typescript
            });
            return this;
        }
        function once(event, handler) {
            var onceFunction = function (params) {
                handler(params);
        ...
        function emit(event, params) {
            if (_listeners[event]) {
                (_listeners[event] || []).forEach(function (e) { return e(params); });
            }
            return this;
        }
    ...
    var emitter$1 = createEmitter();

    var appender = (function (base) {
        var children = [];
        var validateComponent = function (c) {
            if (!c || !c.el || typeof c.el.appendChild !== 'function') {
                throw new Error("Invalid component: ".concat(c));
            }
        };
        emitter$1.on('mutate', function (node) {
        ...
        return {
            ...
            appendAfter: function (component) {
                var e_3, _a;
                var args = [];
                for (var _i = 1; _i < arguments.length; _i++) {
                    args[_i - 1] = arguments[_i];
            ...
            remove: function () {
                ...
                (_a = base.parent) === null || _a === void 0 ? void 0 : _a.setChildren(base.parent.getChildren().filter(function (c) { return c !== base; }));
    ...
    var styler = (function (base) { return ({
        ...
        cssClass: function (style, options) {
            ...
            style = Object.keys(style).reduce(function (acc, key) {
                if (key === '&:dark') {
                    acc['&.dark'] = style[key];
                    return acc;
                }
            ...
            emitter$1.on('theme-changed', function (theme) {
                setTimeout(function () {
                    if (theme === 'dark') {
                        base.el.classList.add('dark');
                    }
            ...
            function applyCssClass() {
                var styles = generateStyleString();
                styles.forEach(function (_a) {
                    ...
                    var rule = "".concat(name[0] === '@' ? '' : '.').concat(name, " { ").concat(body, " }");
                    stylesheet.insertRule(rule, stylesheet.cssRules.length);
                    if (name.includes(':active')) {
                        var _name_1 = "".concat(name.replace(':', '-'));
                        var _rule = "".concat(name[0] === '@' ? '' : '.').concat(name.replace(':', '-'), " { ").concat(body, " }");
                        stylesheet.insertRule(_rule, stylesheet.cssRules.length);
                        base.el.addEventListener('touchstart', function () {
                            base.el.classList.add(_name_1);
                        });
                        base.el.addEventListener('touchend', function () {
                            base.el.classList.remove(_name_1);
                        });
                        base.el.addEventListener('touchcancel', function () {
                            base.el.classList.remove(_name_1);
                        });
                    }
            ...
            function generateStyleString() {
                ...
                var normalStyle = styleKeys.reduce(function (styleString, prop) {
                    styleString += getPropValueLine(prop, style);
                    return styleString;
                }, '');
                var specialStyle = otherKeys.reduce(function (styles, prop) {
                    var key = prop.slice(1);
                    var body = generateStyle(style[prop]);
                    styles.push({ name: name + key, body: body });
                    return styles;
                }, []);
                var mediaStyle = mediaKeys.reduce(function (styles, prop) {
                    var body = generateStyle(style[prop]);
                    styles.push({ name: prop, body: ".".concat(name, " { ").concat(body, " }") });
                    return styles;
                }, []);
                return __spreadArray(__spreadArray([
                    { name: name, body: normalStyle }
                ], __read(specialStyle), false), __read(mediaStyle), false);
            }
            ...
            function getPropValueLine(prop, obj) {
                ...
                if (snake.startsWith('webkit'))
                    snake = '-' + snake;
                var value = sanitizeStyleValue(typeof obj[prop] == 'function' ? obj[prop]() : obj[prop]);
    ...
    function Base(name) {
        ...
        var id = nextId();
        var el = document.createElement(name); // Todo: consider createDocumentFragment
    ...
    function matchRoute(pathname, routes, root) {
        return Object.keys(routes).find(function (route) {
            var pattern = createRoutePattern(root + route).regex;
    ...
    function shouldSkipNavigation(_view, routeKey, params, query) {
    ...
    var Y = function (y) { return ({
        transform: "translateY(".concat(y).concat(typeof y === 'number' ? 'px' : '', ")"),
    }); };
    ...
    function replacePersianDigits(value) {
        if (value === void 0) { value = ''; }
        return value.toString().replace(/[۰-۹]/g, function (d) { return String.fromCharCode(d.charCodeAt(0) - 1728); });
    }
    // strings like 01 -> ۰۱
    function replaceLatinDigits(value) {
        if (value === void 0) { value = ''; }
        return value.toString().replace(/[0-9]/g, function (d) { return String.fromCharCode(d.charCodeAt(0) + 1728); });
    }
    function isRTL$1(char) {
        return /[\u0600-\u06FF]/.test(char);
    }
    ...
    function formatToInternationalPhoneNumber(phoneNumber) {
        var englishDigitsPhone = replacePersianDigits(phoneNumber);
        var phoneWithoutPlus = englishDigitsPhone.replace(/^\+/, '');
        if (phoneWithoutPlus.length == 11)
            return phoneWithoutPlus.replace(/^0?9/, '989');
        return phoneWithoutPlus;
    }
    var locale = {
        replaceLatinDigits: replaceLatinDigits,
        replacePersianDigits: replacePersianDigits,
        // digitToPersian,
        isRTL: isRTL$1,
        jMonths: jMonths,
        formatToInternationalPhoneNumber: formatToInternationalPhoneNumber
    };

    var styles = {
        PAGE_TRANSITION_DURATION: 160,
        PAGE_BASE: __assign(__assign(__assign(__assign(__assign({}, HIDE), Y(60)), ABSOLUTE), EASE(.12)), { top: '0', paddingTop: 'calc(env(safe-area-inset-top) + 0px)', zIndex: '9999', willChange: 'opacity,transform' }),
        PAGE_EXIT: __assign(__assign({}, HIDE), Y(60)),
        PAGE_ENTER: __assign(__assign({}, SHOW), Y(0)),
        PAGE_EXIT_UP: __assign(__assign({}, HIDE), Y(-60)),
    ...
    var AboutPage = function () {
        ...
        base.cssClass(__assign(__assign({}, helpers.styles.PAGE_BASE), { display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', overflowY: 'auto' }));
    ...
    var idb = (function (dbName) { return ({
        ...
        createindex: function (_store, version, index, options) {
            return new Promise(function (resolve, reject) {
                ...
                request.onupgradeneeded = function (event) {
                    ...
                    target.result;
                    var upgradeTransaction = event.currentTarget.transaction;
                    ...
                    return [2 /*return*/, new Promise(function (resolve, reject) {
                            ...
                            request.onsuccess = function () {
                                ...
                                if (!Array.isArray(object))
                                    object = [object];
                                var addedObjects = object.map(function (o) { return objectStore.add(o); });
                                transaction.oncomplete = function () {
                                    addedObjects.map(function (r) { return r.result; });
                                    db.close();
                ...
                return __generator(this, function (_a) {
                    return [2 /*return*/, new Promise(function (resolve, reject) {
                            ...
                            request.onsuccess = function () {
                                var db = request.result;
                                var transaction = db.transaction(store, 'readwrite');
                                var otransaction = transaction.objectStore(store);
                                var clearRequest = otransaction.clear();
        ...
        find: function (store, options) {
            ...
            var filterFn = function (record) {
                ...
                if ((options === null || options === void 0 ? void 0 : options.filter) !== undefined) {
                    ...
                    if (options === null || options === void 0 ? void 0 : options.index) {
                        ...
                        if ((options === null || options === void 0 ? void 0 : options.value) !== undefined) {
                            ...
                            if (options.value instanceof Date) {
                                var startDate = new Date(options.value);
                                startDate.setHours(0, 0, 0, 0);
                                var endDate = new Date(startDate);
                                endDate.setHours(23, 59, 59, 999);
                                if (options.upperBound && options.lowerBound) {
                                    var lowerBound = new Date(options.lowerBound);
                                    lowerBound.setHours(0, 0, 0, 0);
                                    var upperBound = new Date(options.upperBound);
                        ...
                        cursorRequest = index.openCursor(keyRng, (options === null || options === void 0 ? void 0 : options.reverse) ? 'prev' : 'next');
                    }
                    else {
                        cursorRequest = os.openCursor(null, (options === null || options === void 0 ? void 0 : options.reverse) ? 'prev' : 'next');
                    }
        ...
        count: function (store) {
            return __awaiter$8(this, void 0, void 0, function () {
                return __generator(this, function (_a) {
                    switch (_a.label) {
                        case 0: return [4 /*yield*/, waitUntilDbIsReady()];
                        case 1:
                            _a.sent();
                            return [2 /*return*/, new Promise(function (resolve, reject) {
                                    var request = indexedDB.open(dbName);
        ...
        update: function (store, id, payload) {
            return new Promise(function (resolve, reject) {
                var request = indexedDB.open(dbName);
                request.onsuccess = function () {
                    var db = request.result;
                    if (!db.objectStoreNames.contains(store)) {
                        db.close();
                        return reject(new Error("Store ".concat(store, " does not exist.")));
                    }
                    var transaction = db.transaction([store], 'readwrite');
                    var objectStore = transaction.objectStore(store);
                    var reader = objectStore.get(id);
                    ...
                    reader.onsuccess = function () {
                        ...
                        if (!record) {
                            db.close();
                            return reject(new Error("Record with id ".concat(id, " not found.")));
                        }
                        // Merge the record with the payload
                        var updateRequest = objectStore.put(payload);
    ...
    var db = idb('db');
    var init = function () { return __awaiter$8(void 0, void 0, void 0, function () {
        var _a, version, objectStoreNames;
        return __generator(this, function (_b) {
            switch (_b.label) {
                ...
                case 1:
                    _a = _b.sent(), version = _a.version, objectStoreNames = _a.objectStoreNames;
                    if (!!objectStoreNames.contains('clients')) return [3 /*break*/, 3];
                    return [4 /*yield*/, db.createStore('clients', version + 1, {
                ...
                case 2:
                    ...
                    _b.label = 3;
                case 3:
                    if (!!objectStoreNames.contains('appointments')) return [3 /*break*/, 5];
                    return [4 /*yield*/, db.createStore('appointments', version + 2, {
                            keyPath: 'temp_id', indices: [
                                'id',
                                'user_id',
                                'created_at',
                ...
                case 4:
                    ...
                    _b.label = 5;
                case 5:
                    if (!!objectStoreNames.contains('businesses')) return [3 /*break*/, 7];
                    return [4 /*yield*/, db.createStore('businesses', version + 3, {
                            keyPath: 'temp_id', indices: [
                                'id',
                                'user_id',
                                'name',
                ...
                case 6:
                    ...
                    _b.label = 7;
                case 7:
                    if (!!objectStoreNames.contains('transactions')) return [3 /*break*/, 9];
                    return [4 /*yield*/, db.createStore('transactions', version + 4, {
                            keyPath: 'temp_id',
                            indices: [
                                'id',
                                'user_id',
    ...
    function requirePostgrestBuilder () {
    	...
    	// @ts-ignore
    	const node_fetch_1 = __importDefault(browser);
            ...
            try {
                ...
                if (typeof process !== 'undefined' &&
                    ...
                    if (typeof commonjsRequire !== 'undefined') {
                        return commonjsRequire(moduleId);
                    }
                }
                return null;
            }
  ...
  addEventListener("message", (e) => {
    ...
    var SUPABASE_URL = 'https://kjwuyqdprfwdrxygmtmm.supabase.co';
    var SUPABASE_ANON_KEY = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imtqd3V5cWRwcmZ3ZHJ4eWdtdG1tIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NDc1MTU3NTksImV4cCI6MjA2MzA5MTc1OX0.tX4Xp7h5HWHBgVZc4oy6LkFiec__cs15nbKfBwHhFzs';
    ...
    function notifySubscribers() {
        var publicState = {
            token: state$1.token,
            user: state$1.user,
            isAuthenticated: !!state$1.token
        };
        subscribers.forEach(function (callback) { return callback(publicState); });
    }
    function updateState(session) {
        var _a, _b;
        state$1.session = session;
        state$1.token = (_a = session === null || session === void 0 ? void 0 : session.access_token) !== null && _a !== void 0 ? _a : null;
        state$1.user = (_b = session === null || session === void 0 ? void 0 : session.user) !== null && _b !== void 0 ? _b : null;
        notifySubscribers();
    }
    var auth = {
        initialize: function () { return __awaiter$8(void 0, void 0, void 0, function () {
            var _a, session, error, storedSession, error_1;
            return __generator(this, function (_b) {
                switch (_b.label) {
                    case 0:
                        if (state$1.initialized) {
                            console.warn('Auth already initialized');
                            return [2 /*return*/, state$1.session];
                        }
                        _b.label = 1;
                    case 1:
                        _b.trys.push([1, 3, , 4]);
                    ...
                    case 2:
                        ...
                        supabase.auth.onAuthStateChange(function (event, session) { return __awaiter$8(void 0, void 0, void 0, function () {
                            return __generator(this, function (_a) {
                        ...
                        storedSession = localStorage.getItem('sb-kjwuyqdprfwdrxygmtmm-auth-token');
                        console.log('Stored session in localStorage:', storedSession ? 'Present' : 'Not found');
                        return [2 /*return*/, session];
                    case 3:
                        error_1 = _b.sent();
                        console.error('Failed to initialize auth:', error_1);
                        state$1.initialized = true;
                        throw error_1;
        ...
        getToken: function () { return state$1.token; },
        getUser: function () { return state$1.user; },
        getSession: function () { return state$1.session; },
        isAuthenticated: function () { return !!state$1.token; },
        signInWithGoogle: function () { return __awaiter$8(void 0, void 0, void 0, function () {
            var redirectTo, _a, data, error, error_2;
            return __generator(this, function (_b) {
                switch (_b.label) {
                    case 0:
                        _b.trys.push([0, 2, , 3]);
                    ...
                    case 2:
                        error_2 = _b.sent();
                        console.error('Failed to sign in with Google:', error_2);
                        throw error_2;
                    case 3: return [2 /*return*/];
                }
            });
        }); },
        signOut: function () { return __awaiter$8(void 0, void 0, void 0, function () {
            var error, error_3;
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0:
                    ...
                    case 2:
                        error_3 = _a.sent();
                        console.error('Failed to sign out:', error_3);
                        throw error_3;
                    case 3: return [2 /*return*/];
                }
            });
        }); },
        refreshSession: function () { return __awaiter$8(void 0, void 0, void 0, function () {
            var _a, session, error, error_4;
            return __generator(this, function (_b) {
                switch (_b.label) {
                    case 0:
                        _b.trys.push([0, 2, , 3]);
                    ...
                    case 2:
                        error_4 = _b.sent();
                        console.error('Failed to refresh session:', error_4);
                        throw error_4;
                    case 3: return [2 /*return*/];
                }
            });
        ...
        checkSession: function () { return __awaiter$8(void 0, void 0, void 0, function () {
    ...
    function afterLoginMerge() {
        return __awaiter$8(this, void 0, void 0, function () {
            var session, deviceId, _a, error;
            return __generator(this, function (_b) {
                switch (_b.label) {
                    case 0: return [4 /*yield*/, supabase.auth.getSession()];
                    case 1:
                        ...
                        deviceId = utils.device.getId();
                        return [4 /*yield*/, supabase.rpc('claim_device_cards', { p_device_id: deviceId })];
                    case 2:
                        _a = _b.sent(), _a.data, error = _a.error;
                        if (error) {
    ...
    var Input = function (placeholder, type, options) {
        ...
        if (options === void 0) { options = {}; }
        var base = type == 'textarea' ? Base('textarea') : Base('input');
        base.el.setAttribute('type', type);
        base.el.setAttribute('placeholder', placeholder);
        if (options['accept'])
            base.el.setAttribute('accept', options['accept']); // For now
        if ('value' in options)
            base.el.value = options['value'];
        base.el.onblur = function (e) { return base.emit('blur', e); };
        base.el.onfocus = function (e) { return base.emit('focus', e); };
        base.el.oninput = function (e) { return base.emit('input', { e: e, value: base.el.value }); };
        base.el.onclick = function (e) { return base.emit('click', e); };
        base.el.onkeydown = function (v) {
            v.stopPropagation();
            switch (v.key) {
                case 'Enter':
                    base.emit('key-enter', { event: v, value: base.el.value });
                    break;
                case 'Escape':
                    base.emit('key-escape', v);
                    break;
                case 'Tab':
                    base.emit('key-tab', { event: v, value: base.el.value });
                    break;
                case 'Backspace':
                    base.emit('key-backspace', v);
                    break;
                case 'Home':
                    base.emit('key-home', v);
                    break;
                case 'End':
                    base.emit('key-end', v);
                    break;
                case 'PageUp':
                    base.emit('key-page-up', v);
                    break;
                case 'PageDown':
                    base.emit('key-page-down', v);
                    break;
                case 'ArrowUp':
                    base.emit('key-arrow-up', v);
                    break;
                case 'ArrowDown':
                    base.emit('key-arrow-down', v);
                    break;
                default:
    ...
    var DInput = function (title, placeholder, options) {
        if (options === void 0) { options = {}; }
        ...
        var base = Base();
        var t = Div(title);
        var p = Div(placeholder);
        var i = Input('');
        var e = Div(''); // replace with editable component
        base.append(t, p);
        ...
        var firstChar = (placeholder || '').toString().trim().charAt(0);
        i.el.setAttribute('dir', options.dir || (isRTL(firstChar) ? 'rtl' : 'ltr'));
        i.el.setAttribute('type', type);
        if (options.pattern)
            i.el.setAttribute('pattern', options.pattern || '');
        if (options.inputmode)
            i.el.setAttribute('inputmode', options.inputmode || 'text');
        i.on('input', function (payload) {
            if (!options.isLatin)
                i.el.value = helpers.locale.replaceLatinDigits(i.getValue());
            base.emit('input', { value: i.getValue(), event: payload.event });
            p.style({ opacity: (i.getValue().length > 0 ? '0' : '1') });
        });
        e.el.addEventListener('input', function (payload) {
            base.emit('input', e.el.innerHTML);
            p.style({ opacity: (e.el.innerHTML !== '<br>' ? '0' : '1') });
        });
        e.el.addEventListener('paste', function (e) {
            e.preventDefault();
        ...
        if (i.getValue().length > 0)
            p.style({ opacity: '0' });
        i.on('key', function (data) { return base.emit('key', data); });
        i.on('key-enter', function () { return base.emit('key-enter'); });
        i.cssClass({
            height: '46px',
            fontSize: size + 'px',
            width: '100%',
        ...
        p.cssClass(__assign(__assign({}, ABSOLUTE), { height: '46px', top: '57px', right: '0px', left: '0px', width: '100%', textAlign: 'center', fontSize: size + 'px', color: '#00000022', pointerEvents: 'none', zIndex: '0', letterSpacing: options.inputmode === 'numeric' ? '5px' : '0' }));
        setTimeout(function () {
            p.style({
                // Todo: fix this
        ...
        return Object.assign(base, {
            focus: function () {
                var el = options.type === 'textarea' ? e.el : i.el;
            ...
            clear: function () { return options.type === 'textarea' ? e.el.innerHTML = '' : i.clear(); },
            getValue: function () { return options.type === 'textarea' ? e.el.innerHTML : i.getValue(); },
            setValue: function (v) {
                if (v === void 0) { v = ''; }
                if (v.length > 0)
                    p.style({ opacity: '0' });
    ...
    var PageHeader = function (title) {
        if (title === void 0) { title = ''; }
        var base = Div(title);
        base.cssClass(__assign({ position: 'absolute', top: 'env(safe-area-inset-top) + 0px)', right: '0', left: '0', zIndex: '2', height: '60px', backgroundColor: '#fff', fontSize: '18px', fontWeight: 'bold', padding: '18px 0px 17px 60px', textAlign: 'left' }, EASE(.24)));
        return Object.assign(base, {
    ...
    var Page = function () {
        ...
        function exit(_a) {
            var query = _a.query, from = _a.from, to = _a.to, data = _a.data, params = _a.params;
            base.emit('exit', { query: query, from: from, to: to, data: data, params: params });
            base.style(helpers.styles.PAGE_EXIT_UP);
        }
        function enter(_a) {
            return __awaiter$8(this, arguments, void 0, function (_b) {
                var query = _b.query, from = _b.from, to = _b.to, data = _b.data, params = _b.params;
                return __generator(this, function (_c) {
                    switch (_c.label) {
                        case 0: return [4 /*yield*/, waitFor(helpers.styles.PAGE_TRANSITION_DURATION)];
                        case 1:
                            _c.sent();
                            base.emit('enter', { query: query, from: from, to: to, data: data, params: params });
    ...
    function merge(key, value) {
        __state[key] = __assign(__assign({}, __state[key]), value);
    }
    function use(key, defaultValue) {
        if (__state[key] === undefined)
            __state[key] = defaultValue;
        return [
            function () { return __state[key]; },
            function (value) { return __state[key] = value; },
            function (value) { return __state[key] = __assign(__assign({}, __state[key]), value); }
    ...
    var Card = function (_card) {
        ...
        function handleFlip() {
            return __awaiter$8(this, void 0, void 0, function () {
                return __generator(this, function (_a) {
                    switch (_a.label) {
                        case 0: return [4 /*yield*/, waitFor(100)
    ...
    var Timeline = function () {
        ...
        return __assign(__assign({}, base), { load: load });
    };
    ...
    var LoginPage = function () {
        ...
        var wizard = Base();
        base.append(wizard);
        wizard.on('done', function () { return __awaiter$8(void 0, void 0, void 0, function () {
            return __generator(this, function (_a) {
                router.getQuery('redirect');
    ...
    function Scrollable(base) {
        ...
        var scrollOffset = 5; // add a small offset
    ...
    var Body = function (adjustedHeight) {
    ...
    var MenuPage = function () {
        var base = Div();
        var title = PageHeader('Liu Cards');
        var body = Body();
        ...
        login.el.addEventListener('click', function () { return __awaiter$8(void 0, void 0, void 0, function () {
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0: return [4 /*yield*/, services.supabase.auth.signInWithGoogle()];
                    case 1:
                        _a.sent();
                        console.log('Logged in now apply user id');
                        return [4 /*yield*/, services.supabase.afterLoginMerge()];
                    case 2:
                        _a.sent();
                        return [2 /*return*/];
                }
        ...
        logout.el.addEventListener('click', function () { return __awaiter$8(void 0, void 0, void 0, function () {
            return __generator(this, function (_a) {
                switch (_a.label) {
                    case 0: return [4 /*yield*/, services.supabase.auth.signOut()];
                    case 1:
                        _a.sent();
                        handleButtons();
                        return [2 /*return*/];
        ...
        function handleButtons() {
            var user = services.supabase.auth.getUser();
            if (user) {
                welcome.text("Welcome, ".concat(user.email));
        ...
        return __assign(__assign({}, base), { exit: function (_a) {
                ...
                return __awaiter$8(this, arguments, void 0, function (_b) {
                    var _c = _b.from, from = _c === void 0 ? '' : _c;
                    return __generator(this, function (_d) {
                        switch (_d.label) {
                            case 0: return [4 /*yield*/, waitFor(200)];
                            case 1:
                                _d.sent();
                                if (from === '/') {
                                    base.style(__assign(__assign({}, helpers.styles.PAGE_EXIT), EASE(0)));
                                }
                                base.style(__assign(__assign({}, helpers.styles.PAGE_ENTER), EASE(.16)), 50);
                                return [2 /*return*/];
                        }
    ...
    var BackIcon = function () {
        ...
        withRipple(base);
        base.cssClass(__assign({ position: 'absolute', left: "0", top: 'env(safe-area-inset-top) + 0px)', margin: '20px', width: '22px', height: '22px', transition: 'all .28s', zIndex: '99999', opacity: '0', transform: 'translateY(50px) rotateX(180deg)', cursor: 'pointer', '&.hover': {
                opacity: '.8'
            } }, CENTER));
        function show() {
            base.style({
    ...
    var MenuIcon = function () {
        ...
        base.cssClass(__assign(__assign(__assign(__assign({}, ROUND), { left: '0', width: '60px', height: '60px', padding: '20px', transition: 'all .16s', zIndex: '99999' }), CENTER), { alignItems: 'start', flexDirection: 'column', justifyContent: 'space-between', 
            // backgroundColor: 'white',
            position: 'absolute', top: 'env(safe-area-inset-top) + 0px)', cursor: 'pointer', 
            // backgroundColor: '#fff',
    ...
    function disableTouchStartPassive() {
        document.addEventListener('touchstart', function () { return false; }, { passive: true });
    }
    function disableContextMenu(options) {
        if ((options === null || options === void 0 ? void 0 : options.touch) && isTouchDevice()) {
    ...
    (function () { return __awaiter$8(void 0, void 0, void 0, function () {
        ...
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0:
                    window.onerror = function (message, source, lineno, colno, error) { return alert(message); };
                    // await db.init()
                    disableContextMenu({ touch: true});
                    disableTouchStartPassive();
                    document.body.appendChild(app.el);
                    window.ononline = function () { return app.emit('online'); };
                    cards = ldb.get('liucards-cards') || [];
                    _b.label = 1;
                case 1:
                    ...
                    _b.label = 2;
                case 2:
                    if (!!cards_1_1.done) return [3 /*break*/, 5];
                    card = cards_1_1.value;
                    payload = {
                        front: card.front,
                        back: card.back,
                    };
                    return [4 /*yield*/, services.cards.save(payload)];
                case 3:
                    _b.sent();
                    _b.label = 4;
                case 4:
                ...
                case 8:
                    ...
                    return [4 /*yield*/, services.supabase.auth.initialize()];
                case 9:
                    session = _b.sent();
                    if (session) {
                    ...
                    if (services.supabase.auth.isAuthenticated()) {
                        token = services.supabase.auth.getToken();
